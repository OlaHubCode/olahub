...
class OlaHubLandingPageController extends BaseController
{
    ...
    public function getTrendingData()
    {
        ...
        $data = [];
            // $test = \OlaHub\UserPortal\Models\Following::where('user_id',app('session')->get('tempID'))->where('type',3)->with('category')->first();
        if(app('session')->get('tempID')) {
            ...
            // }
            var_dump($followedCategory);return ' ';
            $cItems = \OlaHub\UserPortal\Models\CatalogItem::whereHas('quantityData', function ($q) {
                ...
                ->whereIN('category_id', $followedCategory)
                ->groupBy('store_id')
                // ->paginate(10);
                ->orderByRaw("RAND()")
                ->take(6)
            ...
            $oItems = \OlaHub\UserPortal\Models\CatalogItem::join('catalog_item_occasions', 'catalog_item_occasions.item_id', 'catalog_items.id')
                ...
                ->whereIn('occasion_id', $followedOccasion)
                ->groupBy('store_id')
                // ->paginate(10);
                ->orderByRaw("RAND()")
                ->take(6)
            ...
            $oItems = \OlaHub\UserPortal\Models\CatalogItem::whereHas('quantityData', function ($q) {
                ...
                ->whereIn('brand_id', $followedBrands)
                ->groupBy('store_id')
                // ->paginate(10);
                ->orderByRaw("RAND()")
                ->take(6)
