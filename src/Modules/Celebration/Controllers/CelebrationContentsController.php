...
class CelebrationContentsController extends BaseController
{
    ...
    public function removeCelebrationVideo()
    {
        ...
        if (isset($this->requestData) && count($this->requestData) > 0) {
            ...
            if (!$participant) {
                return response(['status' => false, 'msg' => 'removeViedoToCelebration', 'code' => 400], 200);
            }
            if (!empty($this->requestData['mediaId'])) {
                \OlaHub\UserPortal\Models\CelebrationContentsModel::where('id', $this->requestData['mediaId'])->delete();
            } else {
                \OlaHub\UserPortal\Models\CelebrationContentsModel::where('created_by', $this->requestData['celebrationUser'])
                    ->where('celebration_id', $this->requestData['celebrationId'])->delete();
            }
            (new \OlaHub\UserPortal\Helpers\LogHelper)->setLogSessionData(['response' => ['status' => true, 'msg' => 'removeMediaSuccussfully', 'code' => 200]]);
            (new \OlaHub\UserPortal\Helpers\LogHelper)->saveLogSessionData();

            return response(['status' => true, 'msg' => 'removeMediaSuccussfully', 'code' => 200], 200);
        }
        (new \OlaHub\UserPortal\Helpers\LogHelper)->setLogSessionData(['response' => ['status' => false, 'msg' => 'someData', 'code' => 406, 'errorData' => []]]);
        (new \OlaHub\UserPortal\Helpers\LogHelper)->saveLogSessionData();
